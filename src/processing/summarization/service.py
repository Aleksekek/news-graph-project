"""
Сервис для генерации суммаризаций.
Управляет LLM и репозиторием.
"""
...
class SummarizationService:
    ...
    async def generate_hourly_summary(self, hour: datetime) -> Optional[int]:
        ...
        existing = await self.repo.get_for_period(period_start, period_end, "hour")
        if existing:
        ...
        articles = await self.repo.get_smart_articles(
        ...
        prev_summary_data = await self.repo.get_last("hour")
        prev_summary = None
        ...
        # Сохраняем в БД
        summary_id = await self.repo.save(
            period_start=period_start,
            period_end=period_end,
            period_type="hour",
            content=result,
    ...
    async def generate_daily_summary(self, date: datetime) -> Optional[int]:
        ...
        existing = await self.repo.get_for_period(day_start, day_end, "day")
        if existing:
            logger.info(f"Дневная суммаризация за {day_start.date()} уже существует")
            return existing[0]["id"]

        # Собираем часовые суммаризации за день
        hourly_summaries = await self.repo.get_for_period(day_start, day_end, "hour")
...
"""
        ...
        try:
            result_text = await self.llm.raw_request(prompt)
