...
class DeepSeekAnalyzer:
    ...
    async def generate_summary(
        ...
        logger.info(
            f"Запрос к DeepSeek: {len(posts)} постов, период {period_start} - {period_end}"
        )
        ...
        try:
            ...
            logger.info(
                f"DeepSeek ответ: {usage.prompt_tokens} вх, {usage.completion_tokens} вых, "
...
"""
    ...
    def _parse_response(self, response_text: str, usage) -> Dict:
        ...
        try:
            # Очищаем от маркеров кода, если есть
    ...
    async def _raw_request(self, prompt: str) -> str:
