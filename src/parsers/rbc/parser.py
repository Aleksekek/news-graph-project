...
ARCHIVE_AJAX_URL = (
    "https://www.rbc.ru/search/ajax/" "?query=&dateFrom={date}&dateTo={date}&limit=20&page={page}"
)
...
class RbcParser(BaseParser):
    ...
    async def _get_archive_items_api(self, date: datetime, max_results: int) -> list[dict]:
        ...
        for page in range(1, _MAX_ARCHIVE_PAGES + 1):
            ...
            try:
                data = await self._fetch_json(api_url, headers={"Referer": "https://www.rbc.ru/"})
            except Exception as e:
